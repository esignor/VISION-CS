...
The dataset we used is \textit{public gt package}, Figure \ref{fig:image_wd}, consisting of 4256 images, aimed specifically at solving instance segmentation tasks; however, was not divided into train, val and test set. Consequently, we decided to divide it up manually, reserving 3405 images as train set and 851 images as test set. We did not consider it necessary to make a further partition in val set, to keep as many images as possible in the training; and assuming that our models, using weights already pre-trained on \textit{ImageNet}~\cite{imagenet}, were accurate enough not to require \textit{Model Selection}.
...
Each level of the pyramid tries to detect all instances present in an image. A features map/bases are produced for each of these layers, and each will produce a mask. The latter will be combined into a single final mask.}%Ogni livello della piramide cerca di rilevare tutte le istanze presenti in un'immagine. Per ognuno di questi livelli viene prodotta una features map/basi, e ciascuna produrr\`a una maschera. Quest'ultime verranno combinate in un unica maschera finale.}
  \label{fig:FPN}
\noindent
\end{figure}
...
\hline
\begin{tabular}[c]{cc}\text{Mask R-CNN:} \\ \text{ResNet-101-C4} \end{tabular} & \begin{tabular}[c]{ccc}\text{box AP: 30.888}\\ \text{mask AP: 24.562} \end{tabular} & \begin{tabular}[c]{ccc}\text{box AP: 20.459} \\ \text{mask AP: 19.331}\end{tabular} \\
\hline
...
\label{tab1}
\end{table}
The features of Mask R-CNN that we decided to test were C4, DC5 and FPN. From the results shown in Table \ref{tab1}, it appears that the best performing is the ResNet-101-C4 architecture. However, this runs counter to what is defined in Mask R-CNN paper~\cite{Authors1_maskrcnn}, which shows that 4-stage features extraction is less accurate than the bottom-up and top-down approach of FPN~\cite{FPN}. We explained this inconsistency by the computational limitations of Colab, which constrained us to work with only 4000 iterations. Probably, they are too small a number which did not allow us to grasp the real potential and limitations of each feature tested.  
...
\hline
\begin{tabular}[c]{cc}\textbf{BlendMask:} \\ \textbf{ResNet-101-FPN} \end{tabular} & \begin{tabular}[c]{ccc}\textbf{box AP: 31.017}\\ \textbf{mask AP: 26.411} \end{tabular} & \begin{tabular}[c]{ccc}\textbf{box AP: 20.347} \\ \textbf{mask AP: 20.162}\end{tabular} \\
\hline
\begin{tabular}[c]{cc}\text{BlendMask:} \\ \text{ResNet-101-FPN $+$ dcni} \end{tabular} & \begin{tabular}[c]{ccc}\text{box AP: 32.196}\\ \text{mask AP: 27.212}\end{tabular} & \begin{tabular}[c]{ccc}\text{box AP: 20.497}\\ \text{mask AP:  20.236}\end{tabular} \\
\hline
\end{tabular}
\end{center}
...
\label{tab2}
\end{table}
For BlendMask, the features we have considered have always been extracted with FPN; however, we thought it interesting to also test a ResNet backbone with interval 3 convolution (dcni). As can be seen from the results, which we have reported in Table \ref{tab2}, the application of deformation achieves even better results. In this case, the low number of iterations allowed us to obtain sufficiently clear results, probably because the features we tested was always FPN. We still decided to continue the BlendMask experiments with ResNet-101-FPN, for uniformity of comparison with Mask R-CNN.
...
Architecture & \textit{Cityscapes} & \textit{WildDash}\\
\hline\hline
\begin{tabular}[c]{ccc}\textbf{Mask R-CNN:} \\ \textbf{ResNet-101-FPN $+$} \\ \textbf{freeze at 2rd} \end{tabular} & \begin{tabular}[c]{ccc}\textbf{box AP: 28.563}\\ \textbf{mask AP: 24.676}\end{tabular} & \begin{tabular}[c]{ccc}\textbf{box AP: 19.024} \\ \textbf{mask AP: 17.994}\\\end{tabular} \\
\hline
\begin{tabular}[c]{ccc}Mask R-CNN: \\ ResNet-101-FPN $+$ \\ freeze at 3rd \end{tabular} & \begin{tabular}[c]{ccc}box AP: 26.297\\ mask AP: 22.179 \end{tabular} & \begin{tabular}[c]{ccc}box AP: 19.132 \\ mask AP: 18.224\\\end{tabular} \\
\hline
\begin{tabular}[c]{ccc}Mask R-CNN: \\ \text{ResNet-101-FPN $+$} \\ \text{freeze at 4th } \end{tabular} & \begin{tabular}[c]{ccc}\text{box AP: 25.053}\\ \text{mask AP: 21.349}\end{tabular} & \begin{tabular}[c]{ccc}\text{box AP: 19.824} \\ \text{mask AP: 18.448}\end{tabular} \\
\hline
\end{tabular}
\end{center}
\caption{Experiments on \textit{frozen layers} Mask R-CNN.}
\label{tab3}
\end{table}
...
Architecture & \textit{Cityscapes} & \textit{WildDash}\\
\hline\hline
\begin{tabular}[c]{ccc}\textbf{BlendMask:} \\ \textbf{ResNet-101-FPN $+$} \\ \textbf{freeze at 2rd} \end{tabular} & \begin{tabular}[c]{ccc}\textbf{box AP: 31.017}\\ \textbf{mask AP: 26.411}\end{tabular} & \begin{tabular}[c]{ccc}\textbf{box AP: 20.347} \\ \textbf{mask AP: 20.162}\end{tabular} \\
\hline
\begin{tabular}[c]{ccc}BlendMask: \\ ResNet-101-FPN $+$ \\ freeze at 3rd \end{tabular} & \begin{tabular}[c]{ccc}box AP: 28.596\\ mask AP: 23.649 \end{tabular} & \begin{tabular}[c]{ccc}box AP: 18.993 \\ mask AP: 17.802\end{tabular} \\
\hline
\begin{tabular}[c]{ccc}\text{BlendMask:} \\ \text{ResNet-101-FPN $+$} \\ \text{freeze at 4th} \end{tabular} & \begin{tabular}[c]{ccc}\text{box AP: 31.379}\\ \text{mask AP: 26.936} \end{tabular} & \begin{tabular}[c]{ccc}\text{box AP: 21.597} \\ \text{mask AP: 20.537}\end{tabular} \\
\hline
\end{tabular}
\end{center}
\caption{Experiments on \textit{frozen layers} BlendMask.}
\label{tab4}
\end{table}
From Table \ref{tab4} it appears that BlendMask frozen at 4th seems to be the most performing architecture, while this is not so evident for Mask R-CNN, Table \ref{tab3}. These inconsistencies, once again, we related to the low number of iterations. As a consequence, we have guessed that there is a behaviour that seems to be more promising, freeze 4th (which may depend on the large size of conv4); however, we could not obtain indisputable results to prove this phenomenon. Therefore, for us, a good choice when working with fine-tuning is to freeze the first two convolutional stages of the networks.
...
\hline\hline
\begin{tabular}[c]{ll}Mask R-CNN: \end{tabular} & \begin{tabular}[c]{cccc}box AP: 28.563 \\ mask AP: 24.676 \\ train-time: 1:08:28 \\infer-time: 0:05:13\end{tabular} & \begin{tabular}[c]{cccc}box AP: 19.024 \\ mask AP: 17.994 \\ train-time: 1:13:20 \\ infer-time: 0:08:03\end{tabular} \\
\hline
\begin{tabular}[l]{cc}BlendMask: \end{tabular} & \begin{tabular}[l]{cccccccc}box AP: 31.017 \\($+$2.45 AP )\\ mask AP: 26.411 \\($+$1.74 AP, $+$1.4 AP~\cite{Authors2_BlendMask}) \\ train-time: 0:43:01 \\($-$37\%, $-$20\%~\cite{Authors2_BlendMask}) \\infer-time: 0:12:05 \\($+$131\%, $-$80\%~\cite{Authors2_BlendMask}))\end{tabular} & \begin{tabular}[l]{cccccccc}box AP: 20.347 \\($+$1.32 AP)\\ mask AP: 20.162 \\($+$2.17 AP, $+$1.4 AP~\cite{Authors2_BlendMask})\\ train- time: 0:47:19 \\($-$36\%, $-$20\%~\cite{Authors2_BlendMask})\\ infer-time: 0:15:45 \\($+$96\%, $-$80\%~\cite{Authors2_BlendMask}))\end{tabular} \\
\hline
\end{tabular}
\end{center}

\caption{Results \textit{ideal models}: the architecture is ResNet-101-FPN with freeze at 2nd. In brackets are the improvements of \textit{BlendMask ideal model} compared to \textit{Mask R-CNN ideal model}, and follow the improvements in the reference paper~\cite{Authors2_BlendMask}, when exist.}
...
\noindent
\end{figure}
As confirmed also by Table \ref{mytable_ideal_models}, BlendMask is more accurate than Mask R-CNN both in precision and in training time. However, the benefits obtained by our \textit{ideal model} are significantly lower than those expressed in \cite{Authors2_BlendMask}. We attribute this to the fact that we used 1 single GPU and only 2 000 iterations for our ablations.
...
\centering
  ...
  \caption{Comparison between SOLOv2 and other stage approaches. \textit{Source:~\cite{Authors6_SOLOv2}, pag. 2.} Mask R-CNN is box base two-stage approach, the others are box-free or anchor box-free one-stage approach.} %From top to bottom: SOLOv2\cite{Authors6_SOLOv2} and SOLO\cite{solo} are box-free one-stage approch; Mask R-CNN is box-based two-stage approch; TensorMask~\cite{tensormask} is dense sliding-window one-stage approch; YOLACT~\cite{yolact} is one-stage approch predecessor of \textit{BlendMask}~\cite{Authors2_BlendMask}; PolarMask~\cite{polarmask} anchor box-free, single shot instance segmentation one-stage approch; BlendMask~\cite{Authors2_BlendMask} anchor box-free one-stage approch.}
