...
%Di seguito presentiamo gli eseperimenti pi\`u significativi che ci hanno portato a definire gli \textit{ideal models}.
...
\label{tab1}
\end{table}
The features of Mask R-CNN that we decided to test were C4, DC5 and FPN. From the results shown in the Table \ref{tab1}, it appears that the best performing architecture is the \textit{ResNet-101-C4} architecture. However, this runs counter to what is defined in Mask's paper R-CNN~\cite{Authors1_maskrcnn}, which shows that 4-stage features extraction is less accurate than the bottom up and top down approach of FPN~\cite{FPN}. We explained this inconsistency by the computational limitations of Colab, which constrained us to work with only 4 000 iterations. Probably, they are too small a number which did not allow us to grasp the real potential and limitations of each featurs tested.  Proof of this, is the fact that the same experiment, performed several times, could undergo a variation in accuracy of up to 2\%. As a result of the above, we have decided to consider ResNet-101-FPN as the best performing architecture for Mask R-CNN.
...
\label{tab2}
\end{table}
For BlendMask, the features we have considered have always been extracted with FPN; however, we thought it interesting to also test a ResNet backbone with interval 3 convolution (dcni). As can be seen from the results, which we have reported in Table 2, an increase in depth is also accompanied by an increase in accuracy. In addition, the application of deformation achieves even better results. In this case, the low number of iterations allowed us to obtain sufficiently clear results, probably due to the fact that the features we tested was always FPN.  Although the best results were obtained for ResNet-101-FPN $+$ dcni, we still decided to continue the BlendMask experiments with ResNet-101-FTN, for uniformity of comparison with Mask R-CNN.
...
\label{tab4}
\end{table}
Dalla Tabella \ref{tab4} appare come BlendMask con freeze at 4-th sembra essere l'architettura pi\`u performante, invece questo non \`e cosi evidente per \textit{Mask R-CNN}, Tabella \ref{tab3}, soprattutto per \textit{Cityscapes}. Queste incongruenze, ancora una volta, le abbiamo connesse al basso numero di iterazioni, soprattutto nel caso di Mask R-CNN, ove le valutazioni delle diverse architetture danno risultati abbastanza simili. Di conseguenza intuiamo che c'\`e un comportamento che sembra essere pi\`u promettente, freeze 4-th, che pu\`o dipendere dalla dimensione importante di conv4; tuttavia non siamo riusciti a ottenere risultati indiscutibili a prova di questo fenomeno.

\subsection{Decoder}
Per gli esperimenti sul decoder abbiamo confrontato l'architettura ottenuta dai precedenti esperimenti di BlendMask (con ProtoNet~\cite{protonet}) in rapporto a DeepLabv3~\cite{deeplab}.
\begin{table}[H]
\scriptsize
\begin{center}
\begin{tabular}{|c|c|c|}
...
Architecture & \textit{Cityscapes} & \textit{WildDash}\\
\hline\hline
\begin{tabular}[c]{cc}BlendMask: \\ ResNet-101-FPN $+$ \\ ProtoNet  \end{tabular} & \begin{tabular}[c]{ccc}box AP: \\ mask AP:\end{tabular} & \begin{tabular}[c]{ccc}box AP: \\ mask AP:\end{tabular}  \\
\hline
\begin{tabular}[c]{cc}BlendMask R-CNN: \\ ResNet-101-FPN $+$ \\ DeepLabv3  \end{tabular} & \begin{tabular}[c]{ccc}box AP: \\ mask AP:\end{tabular} & \begin{tabular}[c]{ccc}box AP: \\ mask AP:\end{tabular}  \\
\hline
\end{tabular}
\end{center}
\label{mytable_backbone_Blendmask}
\caption{Experiments on \textit{decoder} of BlendMask.}
\end{table}


\subsection{Ideal models}
Al termine dei nostri esperimenti abbiamo selezionato i seguenti \textit{ideal models} di Mask R-CNN e BlendMask, che risolvono il task di \textit{Urban Street View} oggetto del nostro lavoro:
...
\hline\hline
\textbf{Mask R-CNN:} & \begin{tabular}[c]{cccc}box AP: 28.563 \\ mask AP: 24.676 \\ train time: 1:08:28 \\inference time:\end{tabular} & \begin{tabular}[c]{cccc}box AP: 19.024 \\ mask AP: 17.994 \\ train time: 0:28:50 \\ inference time:\end{tabular} \\
\hline
\textbf{BlendMask:} & &\\
\hline
\end{tabular}
\end{center}
\caption{Results \textit{ideal models}.}
