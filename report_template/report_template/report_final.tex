...
\label{tab1}
\end{table}
The features of Mask R-CNN that we decided to test were C4, DC5 and FPN. From the results shown in Table \ref{tab1} appears that the best performing is the ResNet-101-C4 architecture. However this runs counter to what is defined in Mask R-CNN paper~\cite{Authors1_maskrcnn} which shows that 4-stage features extraction is less accurate than the bottom-up and top-down approach of FPN~\cite{FPN}. We explained this inconsistency by the computational limitations of Colab which constrained us to work with only 4000 iterations. Probably they are too small a number which did not allow us to grasp the real potential and limitations of each feature tested.  
