...
As far as the evaluation is concerned we had to settle of the val set for \textit{Cityscapes} dataset. 
...
\label{tab1}
\end{table}
The feature extractors of Mask R-CNN that we decided to test were C4, DC5 and FPN. From the results shown in Table \ref{tab1} appears that the best performing is the ResNet-101-C4 architecture. However this runs counter to what is defined in Mask R-CNN paper~\cite{Authors1_maskrcnn} which shows that 4-stage features extraction is less accurate than the bottom-up and top-down approach of FPN~\cite{FPN}. We explained this inconsistency by the computational limitations of Colab which constrained us to work with only 4000 iterations. Probably this small number did not allow us to grasp the real potential and limitations of each feature tested.  
...
For this reason we have decided to consider ResNet-101-FPN as the best performing architecture for Mask R-CNN.
...
\label{tab2}
\end{table}
For BlendMask the features we have considered have always been extracted with FPN. However we thought it interesting to also test a ResNet backbone with interval 3 convolution (dcni). As can be seen from the results which we have reported in Table \ref{tab2} the application of deformation achieves even better results. In this case the low number of iterations allowed us to obtain sufficiently clear results, probably because the feature extractor we tested was always FPN. We still decided to continue the BlendMask experiments with ResNet-101-FPN for uniformity of comparison with Mask R-CNN.
...
\hline\hline
\begin{tabular}[c]{cc}Mask R-CNN: \end{tabular} & \begin{tabular}[c]{cccc}box AP: 28.563 \\ mask AP: 24.676 \\ train-time: 1:08:28 \\infer-time: 0:05:13\end{tabular} & \begin{tabular}[c]{cccc}box AP: 19.024 \\ mask AP: 17.994 \\ train-time: 1:13:20 \\ infer-time: 0:08:03\end{tabular} \\
\hline
\begin{tabular}[c]{cc}BlendMask: \end{tabular} & \begin{tabular}[c]{cccccccc}box AP: 31.017 \\($+$2.45 AP )\\ mask AP: 26.411 \\($+$1.74 AP, $+$1.4 AP~\cite{Authors2_BlendMask}) \\ train-time: 0:43:01 \\($-$37\%, $-$20\%~\cite{Authors2_BlendMask}) \\infer-time: 0:12:05 \\($+$131\%, $-$80\%~\cite{Authors2_BlendMask})\end{tabular} & \begin{tabular}[c]{cccccccc}box AP: 20.347 \\($+$1.32 AP)\\ mask AP: 20.162 \\($+$2.17 AP, $+$1.4 AP~\cite{Authors2_BlendMask})\\ train- time: 0:47:19 \\($-$36\%, $-$20\%~\cite{Authors2_BlendMask})\\ infer-time: 0:15:45 \\($+$96\%, $-$80\%~\cite{Authors2_BlendMask})\end{tabular} \\
\hline
\end{tabular}
\end{center}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.43\linewidth]{./image/city_mask.png}\includegraphics[width=0.43\linewidth]{./image/city_blend.jpg}
\includegraphics[width=0.43\linewidth]{./image/city_mask2.png}\includegraphics[width=0.43\linewidth]{./image/city_blend2.jpg}
\includegraphics[width=0.43\linewidth]{./image/wd_mask.png}\includegraphics[width=0.43\linewidth]{./image/wd_blend.jpg}
\includegraphics[width=0.43\linewidth]{./image/wd_mask2.png}\includegraphics[width=0.43\linewidth]{./image/wd_blend2.jpg}  
