...
\hline\hline
\begin{tabular}[c]{ll}Mask R-CNN: \end{tabular} & \begin{tabular}[c]{cccc}box AP: 28.563 \\ mask AP: 24.676 \\ train-time: 1:08:28 \\infer-time: 0:05:13\end{tabular} & \begin{tabular}[c]{cccc}box AP: 19.024 \\ mask AP: 17.994 \\ train-time: 0:28:50 \\ infer-time: 0:03:36\end{tabular} \\
\hline
\begin{tabular}[l]{cc}BlendMask: \end{tabular} & \begin{tabular}[l]{cccccccc}box AP: 30.922 \\($+$2.36 AP )\\ mask AP: 25.922 \\($+$1.25 AP, $+$1.4 AP~\cite{Authors2_BlendMask}) \\ train-time: 0:43:01 \\($-$30\%, $-$20\%~\cite{Authors2_BlendMask}) \\infer-time: 0:12:05 \\($+$131\%, $-$80\%~\cite{Authors2_BlendMask}))\end{tabular} & \begin{tabular}[l]{cccccccc}box AP: 20.347 \\($+$1.32 AP)\\ mask AP: 20.162 \\($+$2.17 AP, $+$1.4 AP~\cite{Authors2_BlendMask})\\ train- time: 0:47:19 \\($+$64\%, $-$20\%~\cite{Authors2_BlendMask})\\ infer-time: 0:15:45 \\($+$338\%, $-$80\%~\cite{Authors2_BlendMask}))\end{tabular} \\
\hline
\end{tabular}
\end{center}
...
\centering
  ...
  \caption{Comparison between SOLOv2 and other stage approches. \textit{Source:~\cite{Authors6_SOLOv2}, pag. 2.} \\ Mask R-CNN is box base two-stage approch, the others are box-free/anchor box-free one-stage approch.} %From top to bottom: SOLOv2\cite{Authors6_SOLOv2} and SOLO\cite{solo} are box-free one-stage approch; Mask R-CNN is box-based two-stage approch; TensorMask~\cite{tensormask} is dense sliding-window one-stage approch; YOLACT~\cite{yolact} is one-stage approch predecessor of \textit{BlendMask}~\cite{Authors2_BlendMask}; PolarMask~\cite{polarmask} anchor box-free, single shot instance segmentation one-stage approch; BlendMask~\cite{Authors2_BlendMask} anchor box-free one-stage approch.}
